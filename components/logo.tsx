...
export function Logo({ width = 80, height = 80, className = "", variant = "full", priority = false }: LogoProps) {
  ...
  const getDimensions = () => {
    switch (variant) {
      case "mega":
        return { width: 500, height: 300 } // More balanced mega size
      case "hero":
        return { width: 400, height: 300 } // Better aspect ratio for hero
      case "large":
        return { width: 200, height: 80 } // More consistent large size
      case "nav":
        return { width: 160, height: 64 } // Standardized nav size
      case "full":
        return { width: width || 120, height: height || 48 }
      case "icon":
  ...
  return (
    ...
    >
      ...
      <Image
        ...
        } ${variant === 'nav' ? 'header-logo' : ''} drop-shadow-lg hover:scale-105 transition-transform`}
